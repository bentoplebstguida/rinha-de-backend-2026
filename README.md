...
API de detecção de fraude por busca vetorial (k-NN) usando **starlette + uvicorn + FAISS IVF SQ8**, containerizada em 2 instâncias atrás de **nginx** (load balancer).

## Stack

- **Python 3.12** + **starlette** (ASGI puro, sem overhead de Pydantic) + **uvicorn** (uvloop + httptools)
- **FAISS** `IndexIVFScalarQuantizer SQ8` (nlist=1024, sq8, nprobe=32) — pré-treinado no build
...
nginx:9999 (load balancer)
    ├── api1:8080 (starlette + uvicorn + FAISS IVF SQ8)
    └── api2:8080 (starlette + uvicorn + FAISS IVF SQ8)
```

### Otimizações

- **Índice pré-treinado no build**: o `IndexIVFScalarQuantizer SQ8` é treinado uma vez no `docker build` (Stage 1) e só carregado em runtime via `faiss.read_index`. O `/ready` responde em < 1s.
- **Memória compacta**: IVF SQ8 comprime os 3M vetores para ~65MB no índice, ~5MB de labels. Cabe folgada em 140MB.
...
(O `build` baixa `references.json.gz` (~284MB), treina o índice IVF SQ8 e gera a imagem. Demora uns 5-10 min no primeiro build.)
...
│   ├── preprocess.py      # references.json.gz → IVF SQ8 index + labels.npy
│   ├── requirements.txt
